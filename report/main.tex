...
Different are the analyses one can carry on for this game. As an example, one can analyze the success rate for different weights in the exploratory player, or analyze the cost in terms of total number of moves increasing the grid size. I started from the naive player and for him there is only one analysis I could have done: see if increasing the grid size and keeping everything else fixed the number of total moves increases or not. As expected, it does, as shown in Fig. \ref{fig:1}.
...
I ran 7 simulations for the values of grid size from 20 to 50 with step 5 (always keeping a square for simplicity). 

For the exploratory player instead, many more analyses could have been carried on. The first and most intuitive is the same as before. For this I selected a weight for the previous cell of 0.95, dividing uniformly the rest between the other three neighboring cells. The results are shown in Fig. \ref{fig:2}.

As expected, the number of moves increases but with a slower pace than the naive player, given that he explores more and falls into dangers more often. 
...
We can notice that the player this time effectively looses more often, confirming the expectations, and the number of moves is much lower than before, indicating that the player often encounters the wumpus early in the game.
